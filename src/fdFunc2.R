...
funcdiv_global = function (x, a, original = FALSE){
  # Arguments
  ...
  return(list(FDis = avg.dist.cent, eig = eig, vectors = vectors, 
              dist.cent = dist.cent, uniq.sp = uniq.sp, dist.mat = x.dist))
}
...
# creating FD function that returns functional dispersion of all communities
# one PCoA is done for every network
funcdiv_network = function (x, a, original = FALSE){
  # Arguments
  #   x = species x traits matrix/data frame. all traits need to be numeric and have no NA
  #   a = site x species matrix/data frame
  #   original = whether matrices inputted are original. Default is 'FALSE'
  ...
  for (i in 1:nrow(a)){
    ...
    ## creating distance matrix 'x.dist'========================================
    # distance measure = euclidean
    x.s <- apply(x.p, 2, scale, center = TRUE, scale = TRUE)
    x.dist <- dist(x.s)
    ...
    # distance of species from community i centroid
    dist.pos <- sweep(vectors[, pos , drop = F], 2, centroid[pos]) 
    dist.pos <- rowSums(dist.pos^2)
    if (any(!pos) ){
      dist.neg <- sweep(vectors[, !pos , drop = F], 2, centroid[!pos])
      dist.neg <- rowSums(dist.neg^2)
    }
    else dist.neg <- 0
